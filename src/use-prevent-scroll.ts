...
export function usePreventScroll(options: PreventScrollOptions = {}) {
  let { isDisabled, skipTouchEndHandling } = options;
  ...
  useIsomorphicLayoutEffect(() => {
    ...
    if (preventScrollCount === 1) {
      if (isIOS()) {
        restore = preventScrollMobileSafari({ skipTouchEndHandling });
      } else {
...
function preventScrollMobileSafari({ skipTouchEndHandling }: { skipTouchEndHandling?: boolean }) {
